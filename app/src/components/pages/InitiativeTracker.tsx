        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {visible.map(init => {
              ...
              const openMilestone = init.milestones.find(m => m.status === 'Overdue') || init.milestones.find(m => m.status === 'Pending');

              return (
                <>
                  <tr key={init.id}
                    className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${isOpen ? 'bg-blue-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-gray-500">{init.category}</td>
                    <td className="px-4 py-3 text-gray-600 font-medium">{init.owner}</td>
                    <td className="px-4 py-3">
                      <select value={init.status} onChange={e => updateStatus(init.id, e.target.value as InitiativeStatus)}
                        className="appearance-none bg-transparent cursor-pointer focus:outline-none">
                        <span className={`px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[init.status]}`}>{init.status}</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        {RAGS.map(r => (
                          <button key={r} onClick={() => updateRAG(init.id, r)}
                            className={`w-3 h-3 rounded-full transition-all ${RAG_COLORS[r]} ${init.ragStatus === r ? 'ring-2 ring-offset-1 ring-gray-400 scale-110' : 'opacity-40'}`} />
                        ))}
                    ...
                    <td className="px-4 py-3 w-32">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${pct >= 75 ? 'bg-emerald-500' : pct >= 40 ? 'bg-blue-500' : 'bg-amber-400'}`}
                            style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-gray-500 w-8">{pct.toFixed(0)}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-gray-500">{fmtDate(init.endDate)}</td>
                  </tr>
                  {isOpen && (
                    <tr key={`${init.id}-exp`} className="bg-slate-50 border-b border-gray-100">
                      <td colSpan={10} className="px-6 py-5">
                        <div className="grid grid-cols-3 gap-5">
                          ...
                          <div className="col-span-1">
                            ...
                            <div className="space-y-2">
                              {init.milestones.map(m => (
                                <div key={m.id} className="flex items-start gap-2">
                                  {MS_ICON[m.status]}
                                  <div>
                                    <p className={`text-xs ${m.status === 'Complete' ? 'text-gray-400 line-through' : m.status === 'Overdue' ? 'text-red-600 font-medium' : 'text-gray-700'}`}>
                                      {m.title}
                                    </p>
                            ...
                            {openMilestone && (
                              <div className={`mt-3 p-2 rounded-lg text-xs ${openMilestone.status === 'Overdue' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>
                                <strong>Next:</strong> {openMilestone.title}
                              </div>
                            )}
                          </div>
                          {/* Savings chart */}
            ...
            <div className="p-5 space-y-3 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="number" value={form.targetSaving || ''} onChange={e => setForm(f => ({ ...f, targetSaving: parseFloat(e.target.value) || 0 }))}
                    className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600">Actual Saving ($M)</label>
                  <input type="number" value={form.actualSaving || ''} onChange={e => setForm(f => ({ ...f, actualSaving: parseFloat(e.target.value) || 0 }))}
                    className="mt-1 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
